...
> 上一次审计（独立子代理按证据逐条证伪）推翻了其中两条：仓库里还有本机路径、文档与实际状态不一致。两者均已处理，见「1. 已完成」与「3. 剩余」。
...
| 测试全绿 | `npm test`（含 `pretest` 构建）：5 个文件、**111 项通过**（本地 vitest 3.2.7，2026-09-22 复测）。更早一次同环境读数为 4 个文件 83 项；差额来自其后新增的客户端与投递测试，不是失败项 |
| 构建自包含且输出正确 | `npm run build` 产出 `lib/index.mjs` + `lib/client.js`；已修两处旧隐患：`tsdown` 未指定输出目录会落到 `dist/`、构建脚本用管道捕获子进程输出会在受限沙箱下 EPERM |
...
| 包内容干净 | `npm pack` 实测 **17 个文件 / 约 55 kB**：含 `lib/index.mjs`、`lib/client.js`、`cordis.patch.yml`、`src/*.ts`、`LICENSE`、`README.md`、`README.zh.md`、`package.json`；**不含** `src/client.js` 与 `lib/*.map`。已去掉 `--sourcemap`，避免产物里留悬空的 map 引用 |
| 许可证与忽略规则 | `LICENSE`（MIT，Tungpeng）；`.gitignore` 覆盖 `node_modules/`、`lib/`、`dist/`、`src/client.js`、`.client-strip/`；`.gitattributes` 统一换行 |
| Git 仓库 | `main` 分支、仓库级身份、工作区干净；历史全部由功能/修复/文档提交构成，其中无产物、无依赖、无个人路径（首个提交曾被审计指出历史仍可翻出本机路径，已重做；2026-09-22 对全部历史复扫本机盘符路径、用户名、Tailscale IP 与密钥模式，零命中） |
| CI | `.github/workflows/ci.yml`：pnpm 10 + `--frozen-lockfile`，`typecheck → build → test`（顺序保证构建产物先于测试），矩阵 Linux + Windows，Node 22；`pnpm-lock.yaml` 已入库 |
| 双语文档 | `README.md`（英文，npm 与市场页展示用）+ `README.zh.md`（中文），配置表按代码实际键名更正为 `launchIntervalSeconds`（默认 10，地板 2，旧名 `tickSeconds` 仍兼容） |
...
| U2 | **建 GitHub 仓库**并推送：`github.com/<你的账号>/dsh-offpeak-inbox`，加 `dsh-plugin` topic | 需要你的账号授权；本机未装 `gh` CLI（要装则 `winget install --id GitHub.cli`） |
| U3 | **发布**：`npm publish`（仓库地址已按 `Tungpeng/dsh-offpeak-inbox` 填好，若账号不同需先改 `package.json` 的 repository/homepage/bugs 与 LICENSE 署名） | 同上，且发布不可撤销 |
| U4 | **挂 Release tarball**：`npm pack` 产物重命名为 `dsh-offpeak-inbox.tgz`（**不带版本号**）后上传到 Release | 同 U2；这一步决定市场里的 `tarball:` 链接有效 |
| U5 | **投稿市场**：仓库创建满 **1 天**后，向市场主仓库提 PR 新增 `data/plugins/<owner>__<repo>.yml` | 同 U2，且 PR 用你的账号提交 |
...
| R5 | 提交后自检 | 推 GitHub 后核对 CI 在两个平台都绿、`dsh plugin --profile web add dsh-offpeak-inbox` 在干净环境能装上 |

## 4. 发布流程（可直接照做）

```sh
# 1) 建远程并推送（先补 U2）
git remote add origin https://github.com/<你的账号>/dsh-offpeak-inbox.git
git push -u origin main

# 2) 发布 npm（先补 U1）
npm publish

# 3) 产出 Release 资产（资产名不带版本号，避免 latest/download 链接在下次发版后 404）
npm pack
# 把生成的 dsh-offpeak-inbox-<版本>.tgz 重命名为 dsh-offpeak-inbox.tgz 后上传到 Release

# 4) 验证市场里的 tarball 链接确实可下载，再提投稿 PR
```
...
- **公开前自查**：本目录的文档若含任何用户名或盘符，必须先替换为占位符或移出公开范围（本文件已做前者）。
